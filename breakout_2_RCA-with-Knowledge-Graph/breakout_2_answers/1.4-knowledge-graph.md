...
Go to the search bar in the Entity Explorer and type `Show all Namespaces` - this returns two `kube-system` and `ecommerce-prod`. It won't be `kube-system` so must be `ecommerce-prod`
