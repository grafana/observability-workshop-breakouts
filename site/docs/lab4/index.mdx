...
**It's always on.** Unlike the other labs, the N+1 behavior runs continuously in the banking app, with traffic arriving in cycles a few hours apart.
...
In the aggregated structure for `frontend`, the `GET /api/transactions` request spends nearly all of its time inside the **transaction** service - `banking.v1.TransactionService/ListTransactions` carries the bulk of it (in one capture, ~128ms of a ~161ms request), with its database spans (`db.Query`, plus `db.Prepare`/`stmt.Query`) rolled up on the critical path rather than one slow operation. The request's latency varies - anywhere from tens of milliseconds to a couple hundred - but wherever it lands, the transaction service's DB layer is where the time goes.

One thing this view *can't* show you: it aggregates spans across traces, rolling repeated identical spans together - so the sheer **count** of queries per request is invisible here (that single `db.Query` bar is all the repeats summed). The repetition is easiest to see in one trace - that's the next question.

**How to find it:**

1. Keep the `resource.service.namespace = banking-prod` filter (from Question 1) and the **Duration** metric selected; open the **Root cause latency** tab.
2. In the structure for `frontend`, find `GET /api/transactions` and note the time concentrated under the transaction service's `ListTransactions` span and its `db.Query` children.
3. If `/api/transactions` isn't there - when banking traffic is light the transaction requests can fall below the auto-selected "slowest traces" band - **drag-select a lower band** on the Duration histogram to pull them in.
...
Opening a `GET /api/transactions` trace shows the transaction service firing the same span over and over - **25-plus `db.Query` repeats** (this one had 27), each tiny (well under a millisecond up to ~1ms). Expand one and the `db.statement` is an identical parameterized lookup:
