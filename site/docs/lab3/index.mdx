...
A k6 load test drives steady traffic. The **`productCatalogResponseCache`** feature flag turns on an in-memory response cache in `productcatalogservice` that grows without bound. Memory climbs into the pod's limit until it is OOMKilled and crash-loops, throwing 500s across the store. Expect several services to look unhealthy at once: the crash-looping pod is the cause, the rest are collateral.

It runs automatically **every day during a default window of 12:00-13:00 UTC**. The feature flag flips on at the start of the window - that flip is the "first event" you'll be looking for on the RCA timeline.
...
The earliest insight is a change event on **`flagd`**, the feature-flag service (an **Amend** insight, shown in blue). Its short blue bar starts before every red band on the timeline, right before `productcatalogservice`'s failure begins.
...
You identified the source entity from its own vs propagated insights (Entity Catalog) and saw the blast radius from failing service to frontend (Entity Graph). You confirmed the OOMKill and right-sizing fix (Kubernetes Monitoring), found the feature-flag change that started it all (RCA Workbench), confirmed the crash in logs and its impact in traces (Drilldown), and verified the Assistant's analysis against the same timeline.
