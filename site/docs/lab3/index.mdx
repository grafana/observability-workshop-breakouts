...
A k6 load test drives steady traffic while the **`productCatalogResponseCache`** feature flag turns on an in-memory response cache in `productcatalogservice` that grows without bound. Memory climbs into the pod's limit until it is OOMKilled and crash-loops, throwing 500s across the store. Expect several services to look unhealthy at once - the crash-looping pod is the cause, the rest are collateral.
