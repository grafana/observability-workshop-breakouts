...
It runs automatically **every day during a default window of 09:00-10:00 UTC** - the accumulated query history from each run is what fills the Queries Overview, samples, and explain plans.
...
**Step 2 - Read the user action.** The action page shows **Total executions**, **Error rate** (0 - nothing is *failing*, it's just slow), and a **Duration p95 in the seconds**, with a chart where the duration jumps at the start of the window. This is the customer-facing symptom: opening a product detail page went from ~500ms to many seconds - and it keeps getting slower as the orders table grows, into the minutes.
...
**Step 4 - Read the trace.** The slow trace is the `GET /api/recommendations` request - seconds end to end (and much longer as the data grows). The waterfall walks the request from the browser through `frontendproxy` and the `frontend` API route down to the span that consumes essentially the whole request: **`oteldemo.RecommendationService/ListRecommendations`**. That tells you *where* the time goes - the recommendation service and its call to the orders database - but not yet *why*.
...
No. Both pages show "you might also like" recommendations, so both fire the same `GET /api/recommendations` call - and open a trace from *either* action and you land on the same `ListRecommendations` span eating essentially the whole request. Whichever you chose, you're looking at the same root cause. (That's also a preview of the blast radius: one slow query, showing up across multiple customer journeys.)
...
**No - and that's the more useful finding.** On `order_items`, both columns the query touches - **`product_id`** and **`order_id`** - carry the **Indexed** badge (BTREE indexes; they're also the table's foreign keys). The index the query needs already exists.

![Table Schema Details - order_items: product_id and order_id are both indexed](/img/lab2/2.4-table-schema.png)

So why did the Explain Plan (Question 3) still show a full scan? Look back at the SQL: it filters on `UPPER(product_id) IN (...)` and `DATE(order_date) >= ...`. Wrapping an indexed column in a function makes the predicate **non-sargable** - MySQL can't use the BTREE index and falls back to scanning the whole table. An index is worthless if the query is written to defeat it.

So the fix isn't "add the missing index" - it's to stop wrapping the column in a function (or add a functional/expression index matching `UPPER(product_id)`), on top of rewriting the correlated subquery from Question 3. If any indexes are genuinely worth adding, they're on other tables - the Assistant flags candidates in Question 6.
...
You started at a slow product page, followed the `open-product-detail` user action into a trace, and saw the recommendation service's call to the orders database eating essentially the whole request. From there Database Observability let you name the exact query, diagnose it (correlated subquery, full scans, an index defeated by `UPPER()`), confirm it saturated the RDS instance's CPU, and get a concrete fix from the Assistant - which you verified against the plan and schema you'd already read.
