...
A new complaint is coming in about the `ecommerce` store: product pages hang for several seconds before the recommendations appear. It is slow rather than broken, and it started recently. This time the trail does not stop at the browser - it leads through a trace and into the database, which is where the real investigation begins.

In this lab you will use [Database Observability](https://grafana.com/docs/grafana-cloud/monitor-applications/database-observability/). It has been GA since April 2026 and covers MySQL and PostgreSQL, whether self-managed or managed (RDS, Aurora, Cloud SQL, Azure). Grafana Alloy collects the data with no agent required on the database host. You will work through the Queries Overview, per-query metrics, the Explain Plan, Table Schema Details, and the Grafana Assistant's query optimization. Together, these give you both the high-level problem and the specific fix needed.

## Learning objectives

- Correlate frontend symptoms to backend database queries using traces
- Identify expensive queries by their execution profile (high duration, rows examined)
- Use Explain Plans to diagnose full table scans and non-sargable predicates
- Verify fixes with the Grafana Assistant and reconcile them against database metrics
...
There is steady browse and checkout traffic against the store. The `recommendationsFromOrderAnalysis` feature flag switches the recommendation service to build its "you might also like" results with an inefficient query against the orders database. Product pages hang for several seconds while recommendations load. The delay traces down to a single expensive SQL query, not a broken service itself.
...
The complaint comes from the frontend, but the delay originates somewhere downstream. Your job is to follow it there and find the root cause.
...
A query can be a problem for two different reasons: it runs thousands of times with each execution cheap, or each individual run is expensive. These columns tell you which one you are dealing with, and that choice determines the fix.
...
This query has a modest call count but a high average duration, and a huge **Rows Examined** count compared to **Rows Sent**. It reads enormous numbers of rows but returns only a few. This is an expensive-per-run query, not a high-frequency one. That distinction matters for how you fix it.
...
You started at a slow product page and followed the `open-product-detail` user action into a trace, where you saw the recommendation service's call to the orders database take almost the whole request. Database Observability then let you name the exact query and diagnose it: a correlated subquery, full scans, and an index defeated by `UPPER()`. You confirmed it saturated the RDS instance's CPU, which is why the slowdown reached customers. The Assistant gave you a concrete fix, which you verified against the explain plan and schema you had already read. This workflow - trace to query to plan to fix - is the one you will use whenever database performance questions arise.

In Lab 3, the problem has a much wider blast radius across multiple services, and you will use the Knowledge Graph to map it.
