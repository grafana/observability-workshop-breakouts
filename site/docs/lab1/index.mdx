...
# Lab 1 - See what your users see

You are receiving support tickets about the `ecommerce` store. Users report that the site feels sluggish, and product images are slow to appear. When you check the backend dashboards, they look fine - CPU is low, error rates are flat, and every service reports healthy. This is the key challenge: the customer-facing problem is real, but your infrastructure metrics do not show it.

In this lab you will use [Frontend Observability](https://grafana.com/docs/grafana-cloud/monitor-applications/frontend-observability/), Grafana's real user monitoring product, to find problems that only exist in the browser. Backend dashboards alone would miss these. You'll work through Core Web Vitals, per-page performance, errors, user sessions, and the trace correlation that links the frontend to the backend so you can see the full picture.

## Learning objectives

- Understand Core Web Vitals and how they signal real user experience degradation
- Use Frontend Observability to identify frontend performance regressions
- Correlate frontend and backend traces to confirm where time is actually spent
- Distinguish between performance issues and availability issues
...
There is steady user traffic against the store. The `imageSlowLoad` feature flag injects a fault: it adds a 1.5-3 second delay to the banner and product images. On image-heavy pages, you should expect to see Largest Contentful Paint (LCP) in the "poor" range. Page loads still succeed and errors stay flat, so this is a pure performance regression with no backend errors appearing.

It runs automatically every day during a default window of 06:00-08:00 UTC.
...
When you are on call and customers say the ecommerce site feels slow, but every backend dashboard is green, where do you start? At the same place your users do: in the browser.

In Grafana, open the left-hand menu and select **Observability** -> **Frontend**. You will see a list of every application instrumented with Frontend Observability. Click into the `ecommerce` app.

Next, set the time range to cover the slow-images window (06:00-08:00 UTC from the lab intro above). Start the range about an hour before that window so you can see the healthy baseline and the moment the slowdown began. Avoid a much wider range like 24 hours, which would pick up the other lab scenarios and make the picture unclear. Alternatively, <EnvLink appIdKey="frontendAppId" path="/a/grafana-kowalski-app/apps/{appId}" from="06:00" to="08:00">jump straight there</EnvLink>: the link takes you to the right place and time range automatically (needs your environment ID and Frontend Observability app ID from the Welcome page).
...
Before you look for problems, get your baseline. It is important to establish what healthy looks like so you can recognize what is not.
...
Traffic is steady and the **Page Loads** panel is essentially all blue, which means pages are loading successfully. Nothing is failing outright, so this is a performance problem rather than an availability problem. This distinction matters: a broken system requires a different investigation path than a slow one.
...
Grafana surfaces Google's [Core Web Vitals](https://grafana.com/docs/grafana-cloud/monitor-applications/frontend-observability/instrument/web-vitals/) as color-coded tiles - green (good), amber (needs improvement), red (poor) - measured at the 75th percentile of real users. The answer below explains what each vital measures.
...
2. Identify **LCP** as the red one, then note its p75 value against the 2.5s threshold.
...
Whether a regression is everywhere or limited to certain routes is a useful early clue.
...
Frontend Observability can group the same metric by geography. You can then tell a global regression from a regional one, such as a bad CDN edge or a single region that degrades on its own.
...
It is easy to assume that "slow" and "broken" go together, but they are separate problems requiring different fixes.
...
Exceptions are flat, so there is nothing new in **Top Exceptions** that lines up with the slowdown. The pages render correctly. They are just slow.

This matters because there is no error to catch here - no 500 in a log, no failing span. The only evidence of a real, revenue-affecting problem is a Web Vital measured in the browser. Without real user monitoring, this incident would be invisible to you.
...
Each session is one visitor's timeline - navigations, network calls, errors, traces, and a video replay of the user's experience (session replay).

<TryIt where="the Sessions tab. Click a Session ID to open the session. Its Activity section lists every event, starting with the Page Load event. You can also watch the session replay to see the page as the user did." />
...
In the session's **Activity** section, click the event whose **Type** is **Page Load** (the first row). The drawer's **Performance** tab breaks the load into **Summary**, **Network**, and **Render**. TTFB and the network phases (DNS, TCP, TLS, request, response) finish in milliseconds and add up to a few percent. **Render** takes 90%+ of the time. The browser spends seconds laying out the page while its images arrive. Close the drawer, and the **HTTP** events for `/images/products/...` carry the inflated durations, alongside a late FCP.
...
Frontend Observability links browser activity to backend traces, so you can see exactly where time is spent. Before calling this a frontend issue, confirm it with a trace.
...
The application is fast. Open an API request from a session and view its trace: the frontend server, API route, and downstream services each return in milliseconds. Trace one of the multi-second `/images/products/...` requests and the same picture holds. The actual application work is a few milliseconds. The seconds sit on the proxy span in front of the app, because the image responses are delayed (the `imageSlowLoad` fault from the lab intro). Nothing in the backend services or database is slow. The regression lives between the user's browser and the application.
...
You found a customer-facing problem that never showed up in a backend dashboard: a slow-image regression visible only through real user monitoring. You confirmed the LCP regression, narrowed it to the image-heavy pages, ruled out errors, and pinned the slow image requests in a real user session. A trace then proved the application itself was healthy, which means the delay sits in front of it at the image response level. You also saw how the Assistant reads a trace and reaches the same conclusion, and you verified that conclusion against your own findings. This is the pattern you will repeat across the labs: find the signal, trace it to the source, and verify with the tools available.
