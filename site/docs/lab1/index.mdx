...
Support tickets are coming in about the `ecommerce` store: the site feels sluggish, and product images are slow to appear. Backend dashboards look fine - CPU is low, error rates are flat, and every service reports healthy.

In this lab you'll use [Frontend Observability](https://grafana.com/docs/grafana-cloud/monitor-applications/frontend-observability/), Grafana's real user monitoring product, powered by the [Faro Web SDK](https://grafana.com/docs/grafana-cloud/monitor-applications/frontend-observability/instrument/faro/). You'll find a problem that only exists in the browser. You'll work through Core Web Vitals, per-page performance, errors, user sessions, and the trace correlation that links the frontend to the backend.
...
A k6 load test drives steady user traffic against the store. The **`imageSlowLoad`** feature flag injects the fault: it adds a 1.5-3 second delay to the banner and product images. Expect Largest Contentful Paint (LCP) to be in the "poor" range on image-heavy pages. Page loads still succeed and errors stay flat, so this is a pure performance regression with no backend errors.
...
In Grafana, open the left-hand menu and select **Observability** -> **Frontend**. You'll see a list of every application instrumented with Faro. Click into the `ecommerce` app.

Then either set the time range yourself - cover **the slow-images window** (06:00-08:00 UTC - see the lab intro), starting about an hour before it so you can see the healthy baseline and the moment the slowdown began, and avoiding a much wider range like 24 hours, which would pick up the other lab scenarios and muddy the picture - or <EnvLink appIdKey="frontendAppId" path="/a/grafana-kowalski-app/apps/{appId}" from="06:00" to="08:00">jump straight there</EnvLink>: the link takes you to the right place and time range automatically (needs your environment ID and Frontend Observability app ID from the Welcome page).
...
Before you look for problems, get your baseline.
...
It's easy to assume that "slow" and "broken" go together.
...
No. Exceptions are flat - **Top Exceptions** shows nothing new that lines up with the slowdown. The pages render correctly. They're just slow.
...
You found a customer-facing problem that never showed up in a backend dashboard: a slow-image regression visible only through real user monitoring. You confirmed the LCP regression, narrowed it to the image-heavy pages, ruled out errors, and pinned the slow image requests in a real user session. A trace then proved the application itself was healthy: the delay sits in front of it, on the image responses. You also saw how **Explain in Assistant** reads a trace and reaches the same conclusion, which you verified against your own findings.
